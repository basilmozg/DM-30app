...
static void (*ASDU_final_maker[ASDU_function_count])(uint8_t *, uint32_t, int16_t, int16_t, uint8_t, uint8_t *);
...
void IEC_real_to_CP56(uint8_t *cp56_part, uint8_t *len)
{
    int16_t loc_msec;
...
void IEC_real_to_CP24(uint8_t *cp56_part, uint8_t *len)
{
    int16_t loc_msec;
    RTC_millis_get(&loc_msec);
    loc_msec += *(modbus_reg_04 + mod_reg_04_RTC_second) * 1000;
...
/*M_SP_NA <1>*/
void M_SP_NA_1_maker(uint8_t *data, uint32_t val_adr, int16_t T_G_value, int16_t T_G_scale, uint8_t QDP, uint8_t *len)
{
    *(data + *len)     =  val_adr        & 255;
    *(data + *len + 1) = (val_adr >> 8)  & 255;
    *(data + *len + 2) = (val_adr >> 16) & 255;
    *(data + *len + 3) = T_G_value;
    *len = *len + 4;
}

/*M_SP_TA <2>*/
void M_SP_TA_1_maker(uint8_t *data, uint32_t val_adr, int16_t T_G_value, int16_t T_G_scale, uint8_t QDP, uint8_t *len)
{
    M_SP_NA_1_maker(data, val_adr, T_G_value, T_G_scale, QDP, len);
    IEC_real_to_CP24(data + *len, len);
}

/*M_ME_NC <13>*/
void M_ME_NC_1_maker(uint8_t *data, uint32_t val_adr, int16_t T_G_value, int16_t T_G_scale, uint8_t QDP, uint8_t *len)
{
    float f_data = (float) T_G_value / T_G_scale;
    uint8_t *value = (uint8_t *) &f_data;
    *(data + *len)     =  val_adr        & 255;
    *(data + *len + 1) = (val_adr >> 8)  & 255;
    *(data + *len + 2) = (val_adr >> 16) & 255;
    *(data + *len + 3) = *value;
...
/*M_ME_TF <36>*/
void M_ME_TF_1_maker(uint8_t *data, uint32_t val_adr, int16_t T_G_value, int16_t T_G_scale, uint8_t QDP, uint8_t *len)
{
    M_ME_NC_1_maker(data, val_adr, T_G_value, T_G_scale, QDP, len);
    IEC_real_to_CP56(data + *len, len);
}

/*M_EI_NA <70>*/
void M_EI_NA_1_maker(uint8_t *data, uint32_t val_adr, int16_t T_G_value, int16_t T_G_scale, uint8_t QDP, uint8_t *len)
{
    *(data + *len)     = 0;
    *(data + *len + 1) = 0;
    *(data + *len + 2) = 0;
...
/*C_IC_NA <100>*/
void C_IC_NA_1_maker(uint8_t *data, uint32_t val_adr, int16_t T_G_value, int16_t T_G_scale, uint8_t QDP, uint8_t *len)
{
    *(data + *len)     = 0;
    *(data + *len + 1) = 0;
    *(data + *len + 2) = 0;
...
void GROUP_answer_04(uint8_t  ASDU_function_type,
                     uint8_t  ASDU_type,
                     uint8_t  NoO,
                     uint8_t  CoT,
                     uint8_t  ORG,
                     ...
                     uint16_t *Data_Source)
{
    ...
    for(uint8_t i = 0; i < NoO; i++)
    {
        (*ASDU_final_maker[ASDU_function_type])(&data_out[0], 
                                                T_G_ptr[i], 
                                                *(Data_Source + T_G_ptr[i]),
                                                T_G_ptr_scale[i],
                                                QDP, 
                                                &data_len);
    }
...
void GROUP_answer_03(uint8_t  ASDU_function_type,
                     uint8_t  ASDU_type,
                     uint8_t  NoO,
                     uint8_t  CoT,
                     uint8_t  ORG,
...
void GROUP_answer_02(uint8_t  ASDU_function_type,
                     uint8_t  ASDU_type,
                     uint8_t  NoO,
                     uint8_t  CoT,
                     uint8_t  ORG,
                     ...
                     uint16_t *Data_Source)
{
    ...
    uint8_t  data_out[255];
    uint8_t  data_len = 12;
    uint16_t NoO_final = 0;
    for(uint8_t i = 0; i < NoO; i++)
    {
        NoO_final += *(T_G_ptr_scale + i);
        for(uint8_t q = 0; q < *(T_G_ptr_scale + i); q++)
            (*ASDU_final_maker[ASDU_function_type])(&data_out[0], 
                                                    T_G_ptr[i] * 16 + 4000 + q, 
                                                    readBit(*(Data_Source + T_G_ptr[i]), q),
                                                    0,
                                                    QDP, 
                                                    &data_len);
    }

    DUI_maker(&data_out[6], ASDU_type, NoO_final, CoT, ORG, COA);
    APCI_maker_104(&data_out[0], data_len - 2, V_S, V_R);
...
void GROUP_answer_01(uint8_t  ASDU_function_type,
                     uint8_t  ASDU_type,
                     uint8_t  NoO,
                     uint8_t  CoT,
                     uint8_t  ORG,
...
void SPORADIC_answer_02(uint16_t T_G_ptr,
                        uint16_t T_G_ptr_bit)
{
    uint8_t  data_out[255];
    uint8_t  data_len = 12;

    uint8_t  ASDU_function_type = M_SP_NA_1_function;
    uint8_t  ASDU_type = M_SP_NA_1;
    uint8_t  NoO = 1;
    uint8_t  CoT = eIEC870_COT_SPONTAN;
    uint8_t  ORG = IEC_ORG;
    uint16_t COA = 1;
    uint8_t  QDP = 0;

    (*ASDU_final_maker[ASDU_function_type])(&data_out[0], 
                                            T_G_ptr * 16 + 2000 + T_G_ptr_bit, 
                                            readBit(*(modbus_reg_04 + T_G_ptr), T_G_ptr_bit),
                                            0,
                                            QDP, 
                                            &data_len);
...
void IEC_104_handler(struct k_timer *dummy)
{
    ...
    switch (IEC_position)
    {
        case 1:
        {
            GROUP_answer_04(M_EI_NA_1_function,
                            M_EI_NA_1,
                            1,
                            eIEC870_COT_INIT,
                            IEC_ORG,
        ...
        case 100:
        {
            GROUP_answer_04(M_ME_NC_1_function,
                            M_ME_NC_1,
                            TAG_GROUP_1_NUMBER,
                            eIEC870_COT_INROGEN,
                            IEC_ORG,
        ...
        case 103:
        {
            GROUP_answer_03(M_ME_NC_1_function,
                            M_ME_NC_1,
                            TAG_GROUP_4_NUMBER,
                            eIEC870_COT_INROGEN,
                            IEC_ORG,
        ...
        case 107:
        {
            GROUP_answer_02(M_SP_NA_1_function,
                            M_SP_NA_1,
                            TAG_GROUP_8_NUMBER,
                            eIEC870_COT_INROGEN,
                            IEC_ORG,
        ...
        case 109:
        {
            GROUP_answer_01(M_SP_NA_1_function,
                            M_SP_NA_1,
                            TAG_GROUP_10_NUMBER,
                            eIEC870_COT_INROGEN,
                            IEC_ORG,
        ...
        case 200:
        {
            GROUP_answer_04(C_IC_NA_1_function,
                         C_IC_NA_1,
                         1,
...
K_TIMER_DEFINE(IEC_104_handler_timer, IEC_104_handler, NULL);
...
void U_format(uint8_t *data_in)
{
    ...
    if(com_bit_2) 
    {
        ...
        k_timer_start(&IEC_104_handler_timer,  K_MSEC(200), K_MSEC(200));
    }
    if(com_bit_4) 
    {
        data_out[2] = 0x23;
...
void IEC_init(int16_t *modbus_reg_04_, int16_t *modbus_reg_03_, void *EVENT_SEND_)
{
    ...
    ASDU_final_maker[M_SP_NA_1_function] = &M_SP_NA_1_maker;
    ASDU_final_maker[M_SP_TA_1_function] = &M_SP_TA_1_maker;
    ASDU_final_maker[M_ME_NC_1_function] = &M_ME_NC_1_maker;
    ASDU_final_maker[M_ME_TF_1_function] = &M_ME_TF_1_maker;
    ASDU_final_maker[M_EI_NA_1_function] = &M_EI_NA_1_maker;
    ASDU_final_maker[C_IC_NA_1_function] = &C_IC_NA_1_maker;
}
